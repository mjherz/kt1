...
headerfile=['pclass','name','sex','age','sibsp','parch','ticket','fare','cabin','embarked','husband','wife','father','mother','son','daughter','title','secname','fam']

#write 
with open('testp2.csv', 'wb') as tfile:
	file_writer = csv.writer(tfile)
	file_writer.writerow(headerfile)
	for i in range(len(names)):
  		writeline=[pclass[i],fullnames[i],sex[i],age[i],sibsp[i],parch[i],ticket[i],fare[i],cabin[i],embarked[i],husband[i],wife[i],father[i],mother[i],son[i],daughter[i],title[i],secname[i],fam[i]]

...
headerfile=['survived','pclass','name','sex','age','sibsp','parch','ticket','fare','cabin','embarked','husband','wife','father','mother','son','daughter','title','secname','fam']

#write 
with open('trainp2.csv', 'wb') as tfile:
	file_writer = csv.writer(tfile)
	file_writer.writerow(headerfile)
	for i in range(len(names)):
  		writeline=[survived[i],pclass[i],fullnames[i],sex[i],age[i],sibsp[i],parch[i],ticket[i],fare[i],cabin[i],embarked[i],husband[i],wife[i],father[i],mother[i],son[i],daughter[i],title[i],secname[i],fam[i]]
		file_writer.writerow(writeline)

for i in range(len(names)):
	if names[i]==names[13]:
		print i, sex[i],age[i],title[i],parch[i],sibsp[i],father[i],mother[i],husband[i],wife[i],son[i],daughter[i]

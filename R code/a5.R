...
form2<-as.formula("survfact~sex+pclass+fare+fam+sibsp+parch+title+secname")
ccforest2<-train(form2,data=train3,method="cforest",trControl=fitcontrol)
ccforest2$results
varImp(ccforest2)

glmu1<-glmulti(form2,data=train3,family="binomial",method="g")
form3<-as.formula(summary(glmu1)$bestmodel)
rm(glmu1)
...
resprobe1$cforest1<-as.integer(0)
resprobe1$cforest2<-as.integer(0)
resprobe1$glmu<-as.integer(0)
